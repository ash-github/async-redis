...
bool connection::pipelined_send(std::string&& pipelined_cmds, std::vector<reply_cb_t>&& callbacks)
{
  ...
  return
    do_write(pipelined_cmds, [this, cbs = std::move(callbacks)](ssize_t sent_chunk_len) {
      if (sent_chunk_len == 0)
        return disconnect();
...
bool connection::send(const std::string&& command, const reply_cb_t& reply_cb)
{
  ...
  return
    do_write(std::move(command), [this, read_it](ssize_t sent_chunk_len) {
      if (sent_chunk_len == 0)
        return disconnect();
...
void connection::data_received(const char* data, ssize_t len)
{
  if (len == 0)
    return disconnect();
  ...
  while (acc < len && req_queue_.size())
  {
    ...
    acc += parser::base_resp_parser::append_chunk(parser, data + acc, len - acc, is_finished);
